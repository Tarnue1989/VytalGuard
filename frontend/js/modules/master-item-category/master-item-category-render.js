// 📁 master-item-category-render.js – Enterprise-Aligned Renderer (Upgraded)
// ============================================================================
// 🧭 Master Pattern: role-render.js / vital-render.js
// 🔹 Full enterprise consistency (permissions, UI logic, tooltips, exports)
// 🔹 Integrates STATUS_ACTION_MATRIX + buildActionButtons
// 🔹 100% ID-safe (masterItemCategoryTableBody / masterItemCategoryList)
// ============================================================================
...
function getCategoryActionButtons(entry, user) {
  return buildActionButtons({
    module: "master_item_category", // maps to STATUS_ACTION_MATRIX.master_item_category
    status: (entry.status || "").toLowerCase(),
    entryId: entry.id,
    user,
    permissionPrefix: "master_item_categories", // ensures permission key consistency
  });
}
...
export function renderCard(entry, visibleFields, user) {
  const details = visibleFields
    ...
    .map(
      (f) => `
        <p><strong>${FIELD_LABELS_MASTER_ITEM_CATEGORY[f] || f}:</strong> 
...
export function renderList({ entries, visibleFields, viewMode, user }) {
  ...
  const noData = `<tr><td colspan="${visibleFields.length}" class="text-center text-muted py-3">No categories found.</td></tr>`;
    ...
    entries.forEach((entry) => {
      ...
      tr.innerHTML = visibleFields
        .map((f) => {
          const val =
            f === "actions"
              ? `<div class="table-actions export-ignore">${getCategoryActionButtons(entry, user)}</div>`
              : renderValue(entry, f);
