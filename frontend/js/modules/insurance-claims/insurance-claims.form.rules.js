...
export const INSURANCE_CLAIM_FORM_RULES = [
  ...
  {id:"providerInput",message:"Insurance provider is required"},
  {id:"invoiceSelect",message:"Invoice is required"},
  {id:"claimNumber",message:"Claim number is required"},
  {id:"amountClaimed",message:"Amount claimed is required"},
  {id:"currencySelect",message:"Currency is required"},

  /* ================= Financial Breakdown (NEW) ================= */
  {
    id:"invoiceTotal",
    message:"Invoice total must equal Insurance + Patient",
    when:()=>{
      const total=Number(document.getElementById("invoiceTotal")?.value||0);
      const ins=Number(document.getElementById("insuranceAmount")?.value||0);
      const pat=Number(document.getElementById("patientAmount")?.value||0);
      return total && (ins+pat!==total);
    }
  },
