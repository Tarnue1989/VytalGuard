// 📁 master-item-render.js – Enterprise-Aligned Renderer (Final)
// ============================================================================
// 🧭 Master Pattern: master-item-category-render.js / autoBillingRule-render.js
// 🔹 Full enterprise consistency (permissions, UI logic, tooltips, exports)
// 🔹 Integrates STATUS_ACTION_MATRIX + buildActionButtons
// 🔹 Includes Feature Module column (safe UUID + name rendering)
// 🔹 100% ID-safe (masterItemTableBody / masterItemList)
// ============================================================================
...
============================================================ */
function getItemActionButtons(entry, user) {
  return buildActionButtons({
    module: "master_item", // maps to STATUS_ACTION_MATRIX.master_item
    status: (entry.status || "").toLowerCase(),
    entryId: entry.id,
    user,
    permissionPrefix: "master_items", // ensures permission key consistency
  });
}
...
function renderValue(entry, field) {
  switch (field) {
    ...
    case "sample_required":
      return renderBoolean(entry[field]);
    ...
    // ✅ FIXED: Only show Feature Module name
    case "feature_module":
...
export function renderCard(entry, visibleFields, user) {
  const details = visibleFields
    ...
    .map(
      (f) => `
        <p><strong>${FIELD_LABELS_MASTER_ITEM[f] || f}:</strong> 
    ...
    ? `
      <div class="card-footer text-end">
        <div class="table-actions">
          ${getItemActionButtons(entry, user)}
        </div>
      </div>`
    : "";
...
export function renderList({ entries, visibleFields, viewMode, user }) {
  ...
  const noData = `<tr><td colspan="${visibleFields.length}" class="text-center text-muted py-3">No items found.</td></tr>`;
    ...
    entries.forEach((entry) => {
      ...
      tr.innerHTML = visibleFields
        .map((f) => {
          const val =
            f === "actions"
              ? `<div class="table-actions export-ignore">${getItemActionButtons(entry, user)}</div>`
              : renderValue(entry, f);
