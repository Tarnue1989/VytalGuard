// 📁 centralstock-render.js – Entity Card System (CENTRAL STOCK | ENTERPRISE FINAL)
// ============================================================================
// 🧭 FULL MASTER PARITY WITH billableitem-render.js
...
============================================================ */
let sortBy = localStorage.getItem("centralStockSortBy") || "";
let sortDir = localStorage.getItem("centralStockSortDir") || "asc";
...
function toggleSort(field) {
  ...
  localStorage.setItem("centralStockSortBy", sortBy);
  localStorage.setItem("centralStockSortDir", sortDir);

  // 🔗 Bridge to MAIN
  window.setCentralStockSort?.(sortBy, sortDir);
  window.loadCentralStockPage?.(1);
}
...
function getCentralStockActionButtons(entry, user) {
  return buildActionButtons({
    ...
    entryId: entry.id,
    user,
    permissionPrefix: "central_stocks",
  });
}
...
export function renderCard(entry, visibleFields, user) {
  ...
  const header = `
    <div class="entity-card-header">
      <div>
        <div class="entity-secondary">${safe(entry.batch_number)}</div>
        <div class="entity-primary">${safe(entry.masterItem?.name)}</div>
      </div>
      ${
        has("status")
  ...
  if (has("supplier"))
    contextItems.push(`🚚 ${safe(entry.supplier?.name)}`);
  ...
  const body = `
    <div class="entity-card-body">
      ${has("quantity") ? fieldRow("Quantity", entry.quantity) : ""}
      ${has("received_date") ? fieldRow("Received", formatDate(entry.received_date)) : ""}
      ${has("expiry_date") ? fieldRow("Expiry", formatDate(entry.expiry_date)) : ""}
      ${has("is_locked") ? fieldRow("Lock State", renderValue(entry, "is_locked")) : ""}
    </div>
  `;
  ...
  return `
    <div class="entity-card centralstock-card">
      ${header}
      ${context}
      ${body}
      ${audit}
  ...
  if (viewMode === "table") {
    ...
    cardContainer.innerHTML = entries.length
      ? entries
          ...
          .join("")
      : `<p class="text-muted text-center">No central stock records found.</p>`;
...
function setupExportHandlers(entries) {
  ...
  const title = "Central Stock Report";
  ...
  document.getElementById("exportPDFBtn")?.addEventListener("click", () => {
    exportData({
      type: "pdf",
      title,
      selector: ".table-container.active, #centralStockList.active",
