...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (userRole.includes("super")) {
    orgSelect.addEventListener("change", async () => {
      await reloadFacilities(orgSelect.value);
    });
  ...
  /* ============================================================
     👥 3. PATIENT → LOAD DEPOSITS (Readable Format)
  ============================================================ */
  setupSuggestionInputDynamic(
    patientInput,
    patientSuggestions,
    ...
    async (selected) => {
      ...
      if (!selected) {
        setupSelectOptions(depositInput, [], "id", "label", "-- Select Deposit --");
        return;
      }
      ...
      // ⭐ ALWAYS use transaction_ref, NOT UUID or label parsing
      ...
      /* ============================================================
         ON DEPOSIT CHANGE → Use remaining_balance
      ============================================================ */
  ...
  /* ============================================================
     🧾 4. Attach Form Submission Logic
  ============================================================ */
  ...
  async function applyPrefill(entry) {
    try {
      ...
      if (entry.patient) {
        ...
        const deposits = await loadDepositsLite({ patient_id: entry.patient.id });
      ...
      document.querySelector(".card-title").textContent = "Edit Deposit Refund";
      form.querySelector("button[type=submit]").innerHTML =
