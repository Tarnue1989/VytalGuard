...
function resetForm() {
  ...
  // Clear text inputs
  ["depositInput", "refund_amount", "reason"].forEach((id) => {
    const el = document.getElementById(id);
    if (el) el.value = "";
  });

  // Clear hidden inputs
  ["depositId", "patientId"].forEach((id) => {
    const el = document.getElementById(id);
    if (el) el.value = "";
  });
  ...
  // Reset max constraint on amount
  const amt = document.getElementById("refund_amount");
  if (amt) amt.removeAttribute("max");

  console.info("🧹 [Deposit Refund] Form reset complete");
}
