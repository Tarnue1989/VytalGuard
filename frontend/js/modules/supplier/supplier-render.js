// 📁 supplier-render.js – Enterprise-Aligned Master Pattern (Role + Permission Driven)
// ============================================================================
// 🔹 Pattern Source: triage-record-render.js
// 🔹 Includes STATUS_ACTION_MATRIX + buildActionButtons for supplier module
// 🔹 Fully compatible with field visibility + export utilities
// 🔹 All IDs preserved (supplierTableBody, supplierList, dynamicTableHead, etc.)
// ============================================================================
...
export function renderDynamicTableHead(visibleFields) {
  ...
  visibleFields.forEach((field) => {
    ...
    th.textContent = FIELD_LABELS_SUPPLIER[field] || field.replace(/_/g, " ");
...
export function renderCard(entry, visibleFields, user) {
  const details = visibleFields
    ...
    .map(
      (f) => `
        <p><strong>${FIELD_LABELS_SUPPLIER[f] || f}:</strong>
...
export function renderList({ entries, visibleFields, viewMode, user }) {
  ...
  const noData = `<tr><td colspan="${visibleFields.length}" class="text-center text-muted py-3">No suppliers found.</td></tr>`;
  ...
  if (viewMode === "table") {
    ...
    entries.forEach((entry) => {
      ...
      tr.innerHTML = visibleFields
        .map((f) => {
          const val =
            f === "actions"
              ? `<div class="table-actions export-ignore">${getSupplierActionButtons(entry, user)}</div>`
              : renderValue(entry, f);
    ...
    cardContainer.innerHTML = entries.length
      ...
      : `<p class="text-muted text-center py-3">No suppliers found.</p>`;
...
function setupExportHandlers(entries) {
  ...
  const title = "Supplier Records Report";
