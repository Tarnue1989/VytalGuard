...
function validatePharmacyTransactionForm(payload, isEdit) {
  ...
  if (!payload.fulfilled_by_id) errors.push("Pharmacist (fulfilled by) is required");
...
function renderItemsTable() {
  ...
  const renderTypeSelect = (it, idx) =>
    `<select class="form-select form-select-sm type-select" data-idx="${idx}">
      <option value="dispense" ${it.type === "dispense" ? "selected" : ""}>Dispense</option>
      <option value="return" ${it.type === "return" ? "selected" : ""}>Return</option>
    </select>`;

  const renderQty = (it, idx, remaining) =>
    `<input type="number" min="0" max="${remaining}" class="form-control form-control-sm dispense-input"
      data-idx="${idx}" value="${it.dispense_now || 0}">`;
  ...
  container.querySelectorAll(".type-select").forEach((sel) =>
    sel.addEventListener("change", (e) => {
      const idx = parseInt(e.target.dataset.idx, 10);
      transactionItems[idx].type = e.target.value;
    })
  );
  container.querySelectorAll(".dispense-input").forEach((input) =>
    input.addEventListener("input", (e) => {
      const idx = parseInt(e.target.dataset.idx, 10);
      transactionItems[idx].dispense_now = Number(e.target.value) || 0;
    })
  );
...
export async function setupPharmacyTransactionFormSubmission({ form }) {
  ...
  const modeUI = isEdit
    ...
    : {
        ...
        btn: `<i class="ri-add-line me-1"></i> Submit Transaction`,
      };
      ...
      async (selected) => {
        ...
        try {
          ...
          const res = await authFetch(
            `/api/lite/prescriptions?patient_id=${selected.id}&status=issued`
          );
  ...
  form.onsubmit = async (e) => {
    ...
    const payload = {
      ...
      fulfilled_by_id: normalizeUUID(fulfilledByInput.dataset.value),
      fulfillment_date: document.getElementById("fulfillment_date")?.value || null,
      notes: document.getElementById("notes")?.value || "",
      items: transactionItems
        .filter((i) => i.dispense_now > 0 && i.department_stock_id)
        .map((i) => ({
          prescription_item_id: i.prescription_item_id,
