// 📁 vital-render.js – Vital Table & Card Renderers (Enterprise-Aligned)
// ============================================================================
// 🧭 Master Pattern: consultation-render.js
...
function getVitalActionButtons(entry, user) {
  return buildActionButtons({
    module: "vital", // maps to STATUS_ACTION_MATRIX.vital
...
export function renderDynamicTableHead(visibleFields) {
  ...
  visibleFields.forEach((field) => {
    ...
    th.textContent = FIELD_LABELS_VITAL[field] || field.replace(/_/g, " ");
  ...
  switch (field) {
    ...
    case "consultation":
      return entry.consultation
        ? `${entry.consultation.diagnosis || entry.consultation.status}`
        : "—";
    case "admission":
      return entry.admission?.status || "—";
    case "triageRecord":
      return entry.triageRecord?.status || "—";
...
export function renderCard(entry, visibleFields, user) {
  ...
  const details = visibleFields
    ...
    .map(
      (f) => `
        <p><strong>${FIELD_LABELS_VITAL[f] || f}:</strong> 
...
export function renderList({ entries, visibleFields, viewMode, user }) {
  ...
  const noData = `<tr><td colspan="${visibleFields.length}" class="text-center text-muted py-3">No vitals found.</td></tr>`;
  ...
  if (viewMode === "table") {
    ...
    entries.forEach((entry) => {
      ...
      tr.innerHTML = visibleFields
        .map((f) => {
          const val =
            f === "actions"
              ? `<div class="table-actions export-ignore">${getVitalActionButtons(entry, user)}</div>`
              : renderValue(entry, f);
    ...
    cardContainer.innerHTML = entries.length
      ...
      : `<p class="text-muted text-center py-3">No vitals found.</p>`;
