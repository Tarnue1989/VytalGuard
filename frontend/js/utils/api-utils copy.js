...
export function autoPagePermissionKey() {
  ...
  // 🧠 Extract module name correctly (after add-, edit-, etc.)
  let moduleKey = baseName;
  const prefixMatch = baseName.match(/^(add|edit|view|verify|approve|list)-(.+)$/);
  if (prefixMatch && prefixMatch[2]) {
    moduleKey = prefixMatch[2]; // e.g. "centralstock"
  } else {
    moduleKey = baseName.split("-")[0];
  }

  // ✅ Normalize module naming (for permission keys)
  moduleKey = moduleKey
    .replace(/-/g, "_")
    .replace(/centralstock/, "central_stock")
    .replace(/billableitem/, "billable_item")      // 🩵 add this line
  ...
  // ✅ Pluralize
  if (!moduleKey.endsWith("s")) moduleKey += "s";
