...
export const financialService = {
  ...
  async applyRefund({ payment_id, amount, reason, user }) {
    return await sequelize.transaction(async (t) => {
      ...
      // 🔹 Create initial RefundTransaction (pending)
  ...
  async reverseRefund(refund_id, user) {
    ...
    async applyDepositToInvoice({ deposit_id, invoice_id, amount, user }) {
      return await sequelize.transaction(async (t) => {
        ...
        const newRemaining = Math.max(0, (parseFloat(deposit.amount) || 0) - newApplied);
        const newStatus = newRemaining > 0 ? DS.APPLIED : DS.VERIFIED;
    ...
    async voidDeposit({ deposit_id, reason, user }) {
      return await sequelize.transaction(async (t) => {
        ...
        if ([DS.VERIFIED, DS.REVERSED].includes(deposit.status)) {
          throw new Error("❌ Verified or reversed deposits cannot be voided");
        }
        ...
        await logLedger({
          ...
          note: `Deposit voided · Reason: ${reason || "N/A"}`,
          user,
          t,
        });
...
/* ----------------- Discounts ----------------- */
  async createDiscount({
    ...
    return await sequelize.transaction(async (t) => {
      ...
      if (invoice_id) {
        ...
        if (!invoice) throw new Error("❌ Invoice not found");
        if (invoice.is_locked) throw new Error("❌ Cannot apply discount to locked invoice");
      }
      ...
      const duplicate = await db.Discount.findOne({
        where: {
          ...
          type,
          value,
          status: [DISCOUNT_STATUS[1], DISCOUNT_STATUS[3]], // active or finalized
        },
        transaction: t,
      });
      if (duplicate) throw new Error("❌ A similar discount is already active/finalized");
      ...
      const discount = await db.Discount.create(
        {
          ...
          status: DISCOUNT_STATUS[0], // draft
          created_by_id: user?.id,
        },
        { transaction: t, user }
      );
  ...
  async updateDiscount({ id, payload, user }) {
    return await sequelize.transaction(async (t) => {
      ...
      if ([DISCOUNT_STATUS[3], DISCOUNT_STATUS[4]].includes(discount.status)) {
  ...
  async toggleDiscountStatus({ id, user }) {
    return await sequelize.transaction(async (t) => {
      ...
      if (!discount) throw new Error("❌ Discount not found");
      if ([DISCOUNT_STATUS[3], DISCOUNT_STATUS[4]].includes(discount.status)) {
        throw new Error("❌ Cannot toggle finalized/voided discount");
      }

      const newStatus =
        discount.status === DISCOUNT_STATUS[1] ? DISCOUNT_STATUS[2] : DISCOUNT_STATUS[1]; // active ↔ inactive

      await discount.update(
        { status: newStatus, updated_by_id: user?.id },
        { transaction: t, user }
      );
      ...
      if (discount.invoice_id) {
        ...
        await logLedger({
          ...
          note: `Discount toggled to ${newStatus}`,
          user,
          t,
        });
      }
  ...
  async finalizeDiscount({ id, user }) {
    return await sequelize.transaction(async (t) => {
      const discount = await db.Discount.findByPk(id, { transaction: t });
      if (!discount) throw new Error("❌ Discount not found");
      if (discount.status !== DISCOUNT_STATUS[1]) {
        throw new Error("❌ Only active discounts can be finalized");
      }

      await discount.update(
        { status: DISCOUNT_STATUS[3], finalized_by_id: user?.id, finalized_at: new Date() },
        { transaction: t, user }
      );
  ...
  async voidDiscount({ id, reason, user }) {
    return await sequelize.transaction(async (t) => {
      const discount = await db.Discount.findByPk(id, { transaction: t });
      if (!discount) throw new Error("❌ Discount not found");
      if (discount.status === DISCOUNT_STATUS[4]) throw new Error("❌ Discount already voided");

      await discount.update(
        {
          status: DISCOUNT_STATUS[4],
          void_reason: reason || null,
          voided_by_id: user?.id,
          voided_at: new Date(),
        },
  ...
  async restoreDiscount({ id, user }) {
    return await sequelize.transaction(async (t) => {
      ...
      if (!discount) throw new Error("❌ Discount not found");
      if (!discount.deleted_at) throw new Error("❌ Discount is not deleted");

      await discount.restore({ transaction: t });
      await discount.update({ updated_by_id: user?.id }, { transaction: t });
