...
/* ============================================================
   ...
   - No hardcoding
   - No SuperAdmin crash
   - tenant_scope is the single source of truth
============================================================ */
const buildTenantWhere = ({
  module,        // 👈 FeatureModule row (REQUIRED)
  orgId,
  facilities,
  start,
  end,
  ...
  /* ============================================================
     🏢 ORG / FACILITY MODULE
     → org required
     → SuperAdmin gets empty-safe result instead of crash
  ============================================================ */
  if (!orgId) {
...
export const getDashboardData = async (req, res) => {
  try {
    ...
    const start =
      ...
      normalizeDateOnly(new Date(new Date().getFullYear(), new Date().getMonth(), 1));
    ...
    debug.log("CACHE MISS", { cacheKey });

    /* ========================================================
       🔐 Detect SuperAdmin (NO hardcoding elsewhere)
    ======================================================== */
    ...
    /* ========================================================
       🔐 Load FeatureModules (RBAC + GLOBAL SAFE)
       ✔ SuperAdmin → sees GLOBAL modules
       ✔ Org users → strict RBAC
    ======================================================== */
    const modules = await FeatureModule.findAll({
      ...
      where: {
        ...
        deleted_at: null,
        ...(isSuperAdmin
          ? {} // 👑 SuperAdmin sees all (tenant_scope controls data)
          : { "$access.id$": { [Op.ne]: null } } // 🔒 Enforce RBAC
        ),
      },
    ...
    debug.log("MODULES LOADED", { count: modules.length });

    /* ========================================================
       📦 Dashboard-visible modules (DB decides)
    ======================================================== */
    ...
    for (const mod of dashboardModules) {
      ...
      debug.log("KPI RESOLUTION", {
        moduleKey: mod.key,
        tenantScope: mod.tenant_scope,
      });

      const where = buildTenantWhere({
        module: mod,
        orgId: isSuperAdmin ? orgId : orgId,
        facilities: facilityIds,
        start,
        end,
      });

      // 👑 SuperAdmin financial override
      ...
      if (isSuperAdmin && CROSS_TENANT_MODULES.includes(mod.key)) {
    ...
    ======================================================== */
    const alerts = [];
    const labPending = kpis.find(k => k.key === "lab_requests")?.summary?.pending || 0;
    const apptScheduled = kpis.find(k => k.key === "appointments")?.summary?.scheduled || 0;

    if (labPending > 20)
      alerts.push({ type: "warning", message: "High number of pending lab requests." });

    if (apptScheduled > 30)
      alerts.push({ type: "info", message: "Many appointments awaiting start." });
