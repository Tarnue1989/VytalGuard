...
export const register = async (req, res) => {
  ...
  try {
    ...
    const { email, password, role_id, organization_id, facility_id, employee_id, is_default } = req.body;
    ...
    // Create user (now includes organization_id)
    ...
    // 📌 Store initial password in password_history
    ...
    if (roleName === "super admin") {
      console.log("🛡️ Registered a Super Admin (no org/facility link).");
    } else {
    ...
    await auditService.logAction({
      ...
      details: { role: role.name, organization_id, facility_id },
    });
...
export const login = async (req, res) => {
  try {
    ...
    console.log("🧪 [LOGIN] user loaded", {
      id: user.id,
      email: user.email,
      user_org_id: user.organization_id,
      facilityLinks: user.facilityLinks?.map((fl) => ({
        facility_id: fl.facility_id,
        org_id: fl.organization_id,
        role: fl.role?.name,
      })),
    });
    ...
    if (user.locked_until && user.locked_until > new Date()) {
      return res
        .status(403)
        .json({ error: `Account locked until ${user.locked_until.toISOString()}` });
    }
    ...
    // 🔹 Role detection
    const isSystemOwner = user.email?.toLowerCase() === SYSTEM_OWNER_EMAIL;
    const hasSuperAdminRole = user.facilityLinks?.some((fl) =>
    ...
    if (isSystemOwner || hasSuperAdminRole) {
      const superAdminRole =
        (await Role.findOne({ where: { name: { [Op.iLike]: "%super admin%" } } })) ||
      ...
      console.log("🧪 [LOGIN] SUPERADMIN PAYLOAD", payload);
      ...
      payload = {
        ...
        facility_ids: (user.facilityLinks || [])
          .map((fl) => fl.facility_id)
          .filter(Boolean),
        roles: (user.facilityLinks || [])
          .map((fl) => fl.role)
          .filter(Boolean)
          .map((r) => ({
            id: r.id,
            name: r.name,
            normalized: (r.name || "").trim().toLowerCase(),
          })),
      };

      console.log("🧪 [LOGIN] TENANT PAYLOAD", payload);
    }

    // 🔹 Fetch permissions
    const roleIds = payload.roles.map((r) => r.id).filter(Boolean);

    console.log("🧪 [LOGIN] roleIds for permissions", roleIds);
    ...
    console.log("🧪 [LOGIN] FINAL PAYLOAD BEFORE TOKEN", payload);
...
export const forceResetPassword = async (req, res) => {
  try {
    ...
    console.error("forceResetPassword error:", err);
    await auditService.logError({
      module: "auth",
      action: "force_password_reset",
      error: err,
